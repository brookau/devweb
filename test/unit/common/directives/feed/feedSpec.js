define(function (require) {
  ...
  var angular = require('angular'),
    feed = require('app/common/directives/feed/feed'),
    filters = require('app/common/filters'),
    $compile, $scope;

  describe('Feed directive', function () {
    beforeEach(module('ernr.directives.feed'));
    ...
    it('should has feed directive', function () {
      ...
      var element = $compile('<div feed="feed"></div>')($scope);
      $scope.$digest();
      expect(angular.element(element.children()[4]).find('a').text()).to.equal($scope.feed.title);
    });
  });
});
